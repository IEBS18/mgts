...
post_processing_fields = {
    ...
    "pregranted": [
        "pgpub_id", "Title", "Abstract", "Claim", "Publication_Date",
        "Assignee_Applicant", "Inventor", "IPC_Classifications", "CPC_Classifications"
    ],
...
def fuzzy_search(user_query, size=10):
    """Performs fuzzy search across all specified indexes."""
    all_results = []
...
def fetch_raw_results(user_query, size=10):
...
def process_with_llm(results):
    ...
    for patent in results:
        ...
        display_key = patent.get("Display_Key", patent.get("pgpub_id", "N/A"))  # fallback logic
        ...
        extracted_info = {
            ...
            "Claim": claim,
            "Abstract": abstract,
            # Use Publication Number instead of Display_Key
...
def process_with_llm_async(results, user_query):
    """Processes results with LLaMA-3.3 asynchronously and stores output."""
