...
def signup():
    ...
    new_user = {
        ...
        "logins": []  # Initialize empty login history
    }
    users.append(new_user)
    save_users(users)
...
def login():
    ...
    if user and check_password_hash(user['password_hash'], password):
        ...
        # Initialize logins array if it doesn't exist
        if "logins" not in user:
            user["logins"] = []
            
        # Add login record to user's login history
        user["logins"].append(login_record)
        
        # Update last_login field for quick access
        user["last_login"] = login_record
