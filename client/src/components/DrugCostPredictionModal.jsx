...
// src/components/DrugCostPredictionModal.jsx
...
export default function DrugCostPredictionModal({ isOpen, onOpenChange, onSearchSubmit }) {
  const [drugData, setDrugData] = useState({
    drug_name: "",
    // Other fields...
  });
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl bg-[#f4f4f4] rounded-[12px]">
        ...
        <Tabs defaultValue="predict" className="w-full">
          ...
          <TabsContent value="visualize" className="space-y-4 bg-white p-4 rounded-[12px] mt-4">
            <Label htmlFor="drug_name_visualize">Select Drug</Label>
            <Select
              id="drug_name_visualize"
              options={outputData["ACTIVE INGREDIENT"].map(drug => ({ value: drug, label: drug }))}
              value={selectedDrug}
              onChange={handleDrugChange}
              placeholder="Select a Drug"
