...
/**
 * Unify known keys to consistent naming (e.g., "drug_name" => "Drug Name").
 * Then convert each value to a safe/valid string for display.
 */
function unifyAndSanitizeDrugKeys(drugItem) {
  const unified = {};

  // Standardize the keys by converting multiple variations to a single one:
  Object.keys(drugItem).forEach((originalKey) => {
    const lowerKey = originalKey.toLowerCase().trim();
    const val = drugItem[originalKey];

    switch (lowerKey) {
      case "drug name":
      case "drug_name":
        unified["Drug Name"] = val;
        break;
      case "drug type":
      case "drug_type":
        unified["Drug Type"] = val;
        break;
      case "modality":
        unified["Modality"] = val;
        break;
      case "tier":
        unified["Tier"] = val;
        break;
      case "efficacy":
        unified["Efficacy"] = val;
        break;
      case "safety":
        unified["Safety"] = val;
        break;
      case "plan name":
      case "plan_name":
        unified["Plan Name"] = val;
        break;
      case "plan type":
      case "plan_type":
        unified["Plan Type"] = val;
        break;
      case "state name":
      case "state_name":
        unified["State Name"] = val;
        break;
      case "requirements/limits":
      case "requirements":
      case "requirement":
        unified["Requirements/Limits"] = val;
        break;
      case "warnings and precautions for use":
        // You mentioned you don't want to show this, so we skip it.
        break;
      default:
        // Keep any other fields under their original name.
        unified[originalKey] = val;
    }
  });

  // Now sanitize each value (turning null, undefined, or "NaN" into "Not Available").
  const sanitized = {};
  Object.entries(unified).forEach(([key, value]) => {
    sanitized[key] = formatValue(value);
  });
...
/**
 * Flatten & unify the shape of competitor drugs so each item has consistent keys.
 */
const useFlattenedCompetitorDrugs = (list_competitor_drug) => {
  return useMemo(() => {
    if (!list_competitor_drug) return [];
    ...
    Object.keys(list_competitor_drug).forEach((disease) => {
      const drugArray = list_competitor_drug[disease] || [];
      drugArray.forEach((rawDrug) => {
        const sanitizedDrug = unifyAndSanitizeDrugKeys(rawDrug);
        allDrugs.push(sanitizedDrug);
      });
    });
    return allDrugs;
...
const FormularyDrugPage = () => {
  ...
  // Flatten & sanitize once, memoized:
  const flattenedCompetitorDrugs = useFlattenedCompetitorDrugs(
    list_competitor_drug
  );
  ...
  return (
    <div className="w-full p-2">
      ...
      <div className="grid grid-cols-3 gap-4 mt-4">
        {flattenedCompetitorDrugs.map((drugItem, index) => {
          // Safely extract "Drug Name" and "Tier" to display in the card header
          const displayName =
            drugItem["Drug Name"] !== "Not Available"
              ? drugItem["Drug Name"]
              : "Unnamed Drug";
          const displayTier =
            drugItem["Tier"] !== "Not Available" ? drugItem["Tier"] : "";
          ...
          return (
            ...
            >
              ...
              <div className="mt-2">
                ...
                {/* If present, show Modality */}
                {drugItem["Modality"] !== "Not Available" && (
                  <p>
                    <strong className="font-semibold">Modality:</strong>{" "}
                    {drugItem["Modality"]}
                  </p>
                )}
              </div>
          ...
          <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto bg-white scrollbar-hide">
            ...
            <DialogDescription>
              {Object.keys(selectedResult).map((key) => {
                ...
                // Show multi-line text for Efficacy or Safety
                if (lower === "efficacy" || lower === "safety") {
                ...
                // Skip if there's no real information or if it's a hidden field
                if (
                  !value ||
                  value === "Not Available" ||
                  lower === "pdf_name" ||
                  lower === "warnings and precautions for use"
                ) {
                  return null;
                }

                // Convert underscores in the key to spaces for a label
                const label = key.replace(/_/g, " ");
