...
//       navigate('/formulary-competitor',{ state: { /*here i want to add the  list_competitor_drug returning from backend */ } })
//     } catch (error) {
//       console.error("Error:", error);
...
// Utility to handle any invalid or "NaN" values from the backend
...
export function FormularyDashboard() {
  ...
  // Adjust the dropdown style to match text inputs
  const renderSelect = (
    id,
    label,
    options,
  ...
  const handleSubmitDrug = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/add-drug-formulary`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        // Attempt to parse error from server
        let errorMsg = "Failed to add drug.";
        try {
          const errorData = await response.json();
      ...
      // to fix "NaN" parse errors
      // ==============================
      const text = await response.text();

      // Replace unquoted NaN with a string "NaN" (or "null")
      // e.g.  : NaN,  =>  : "NaN",
      //        :NaN} =>  : "NaN"}
      // This handles both trailing commas and end-of-object
      const sanitizedText = text.replace(/:\s*NaN(\s*(,|\}))/g, ': "NaN"$1');

      // Now parse the sanitized text
      const rawData = JSON.parse(sanitizedText);

      // Extract fields
      const list_competitor_drug_raw = rawData.list_competitor_drug || {};
      const drug_name_raw = rawData.drug_name || "";

      // Convert potential NaN fields to "Not Available"
      const sanitizedListCompetitorDrug = {};
      for (const [disease, drugArray] of Object.entries(list_competitor_drug_raw)) {
        sanitizedListCompetitorDrug[disease] = drugArray.map((drugItem) => {
          const sanitizedDrug = {};
          Object.entries(drugItem).forEach(([key, val]) => {
            sanitizedDrug[key] = sanitizeValue(val);
          });
          return sanitizedDrug;
        });
      }

      const sanitizedDrugName = sanitizeValue(drug_name_raw);

      toast.success("Drug added successfully!");
      setIsAddDrugDialogOpen(false);

      // Navigate to /formulary-competitor with sanitized data
      navigate("/formulary-competitor", {
        state: {
          list_competitor_drug: sanitizedListCompetitorDrug,
          drug: sanitizedDrugName,
        },
      });
    } catch (error) {
      console.error("Error:", error);
