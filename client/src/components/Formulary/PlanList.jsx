...
export function PlanList({ plans, states, onSelect, selectedPlans, selectedState, setSelectedState }) {
            ...
            {paginatedPlans.map((plan, index) => (
              <div
                ...
                className={`border rounded-[12px] p-4 cursor-pointer transition-colors duration-200 ${
                  selectedPlans.some((p) => p.id === index)
                    ? "border-green-600 bg-green-100"
                    : "border-gray-300 hover:bg-green-50"
                }`}
                onClick={() => onSelect({ id: index, name: plan })}
              >
                <p className="text-sm font-medium text-center">{plan}</p>
              </div>
            ))}
          </div>
