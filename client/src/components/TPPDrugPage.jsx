...
const TPPDrugPage = () => {
    ...
    const handleComparison = () => {
        ...
        navigate('/tpp-comparison', { state: { comparisonData } });
    };
            ...
            <div className="grid grid-cols-3 gap-4 mt-4">
                ...
                {filteredResults?.map((result, index) => (
                    ...
                    >
                        ...
                        <div className="mt-2">
                            <p className="text-black font-bold"> {result.Drug} {result['Dosage Size'] && `(${result['Dosage Size']})`}</p>
                            {result['Modality'] && (
                                <p>
                    ...
                    <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto bg-white scrollbar-hide">
                        ...
                        <DialogDescription>
                            {Object.keys(selectedResult).map((key) => {
                                ...
                                if (key === "Countries" || key === "Disease" || key === "Dosage Size" || key === "Route Of Administration" || key === "Patient Eligibility") {
                                    value = value.split('\n').map((item, index) => <span key={index}>{item}<br /></span>);
                                }
