...
const Dashboard = () => {
  ...
  const handleCardClick = (key, locked) => {
    // Don't proceed if card is locked
    if (locked) {
      return;
    }
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full bg-gray-50 overflow-auto">
      ...
      {/* Cards Layout */}
      <div className="grid grid-cols-5 gap-4 max-w-screen-xl">
        {reportTypes.map(({ title, description, image, key, locked }) => (
          <motion.div
            key={key}
            className={`relative h-[300px] w-[220px] bg-custom-gradient-card text-white rounded-lg shadow-md overflow-hidden group ${
              locked ? 'cursor-not-allowed' : 'cursor-pointer'
            }`}
            onClick={() => handleCardClick(key, locked)}
            initial={{ background: "linear-gradient(36.59deg, #A6CE39 2.62%, #40530C 92.34%)" }}
            whileHover={
              !locked
                ? {
          ...
          >
            ...
            <div className="p-4 flex flex-col items-center justify-center h-full group-hover:items-start group-hover:justify-start transition-all duration-300">
              ...
              <motion.h3
                className={`text-2xl font-bold mb-2 transition-all duration-300 ${
                  locked ? 'group-hover:text-2xl' : 'group-hover:text-xl'
                }`}
              >
                {title}
              </motion.h3>

              {/* Underline - only show on unlocked cards when hovered */}
              {!locked && (
                <motion.div
                  className="hidden group-hover:block w-10 h-1 bg-white transition-all duration-300"
                ></motion.div>
              ...
              {/* Description (Hidden by default, only show on unlocked cards) */}
              {!locked && (
                <motion.p
                  className="text-sm text-gray-100 opacity-0 group-hover:opacity-100 mt-4 leading-relaxed transition-opacity duration-300"
                >
              ...
              <motion.img
                src={image}
                alt={`${title} icon`}
                className={`h-28 w-28 mt-4 transition-all duration-300 ${
                  locked 
                    ? 'opacity-50' 
                    : 'group-hover:absolute group-hover:bottom-4 group-hover:right-4 group-hover:h-16 group-hover:w-16'
                }`}
              />
            </div>

            {/* Button - only show on unlocked cards */}
            {!locked && (
              <motion.div
                className="hidden group-hover:block absolute bottom-4 left-4"
              >
            ...
            {/* Glass Overlay and Lock Icon for locked cards */}
            {locked && (
              <>
                {/* Glass overlay */}
