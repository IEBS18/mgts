...
//    // 1) detect a comma‑separated list of URLs
//    if (typeof content === "string" && content.includes("https://")) {
//     return content
//       .split(",")               // break on commas
//       .map((url) => url.trim()) // remove extra spaces
//       .filter(Boolean)          // guard against empty entries
//       .map((url, idx) => {
//         // extract PMC ID (PMC followed by digits)
//         const match = url.match(/PMC\\d+/i);
//         const label = match ? match[0] : url; // fallback: whole url
//         return (
...
//               {label}
//             </a>
//             {/* add comma & space except after last link */}
//             {idx < content.split(",").length - 1 && ", "}
//           </React.Fragment>
//         );
//       });
//   }
