...
function renderCellContent(record, topic) {
  ...
  // Handle URLs (similar to DiseaseTable)
...
export default function RNDFormulation() {
  ...
  useEffect(() => {
    ...
    const endpoint =
      `${import.meta.env.VITE_API_URL}/api/rnd-formulation-llama` +
      `?user_query=${encodeURIComponent(userQuery)}` +
      `&size=5` +
      `&requested_fields=${encodeURIComponent(requestedFields)}`
    ...
    eventSource.onmessage = (event) => {
      ...
      try {
        ...
        setTableData((prev) => {
          const existingIndex = prev.findIndex((col) => ensureStableKey(col) === stableKey)
          if (existingIndex !== -1) {
            // Merge new fields
            const updated = [...prev]
            updated[existingIndex] = {
  ...
  return (
    <div className="p-4 bg-gray-100 min-h-screen">
      ...
      <h1 className="text-2xl font-bold mb-4 text-gray-800">R&D Formulation Results for "{userQuery}"</h1>

      <Card className="mb-8 shadow-md">
        <CardHeader className="pb-3 bg-[#f9faf5]">
          <div className="flex items-center justify-between">
            <CardTitle>Formulation Data</CardTitle>
            <Button
              onClick={() => setAiColumnDialogOpen(true)}
              variant="outline"
              className="bg-white text-[#a6ce39] border border-[#a6ce39] hover:bg-[#f0f8e5] rounded-lg"
              disabled={isAiColumnLoading}
            >
              {isAiColumnLoading ? "Loading..." : "Add AI Column"}
            </Button>
              ...
              <table className="w-full min-w-max border-collapse">
                ...
                <tbody>
                  {tableData.length ? (
                    ...
                    <tr>
                      <td colSpan={topics.length} className="p-6 text-center text-muted-foreground">
      ...
      <Dialog open={aiColumnDialogOpen} onOpenChange={setAiColumnDialogOpen}>
        <DialogContent className="bg-white">
          <DialogTitle>Add AI Column</DialogTitle>
          <DialogDescription>Enter a name and description for the new AI column.</DialogDescription>
          <div className="space-y-4 mt-4">
            <Input
              value={aiColumnName}
              onChange={(e) => setAiColumnName(e.target.value)}
