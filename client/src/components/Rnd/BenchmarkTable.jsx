...
import { Loader2, Info, Settings, BarChart3, Download } from "lucide-react"
...
export default function BenchmarkTable({ onWeightsUpdate, initialWeights }) {
  const [benchmarkData, setBenchmarkData] = useState([])
  const [originalBenchmarkData, setOriginalBenchmarkData] = useState([]) // Store original data
  ...
  // New state to track input values separately
  ...
  // Initialize input values from weights
  useEffect(() => {
    const initialInputs = {}
    Object.entries(weights).forEach(([key, value]) => {
      initialInputs[key] = value.toFixed(2)
    })
  ...
  const fetchBenchmarkData = async () => {
    ...
    try {
      ...
      if (data.error) {
        ...
        // Store the original benchmark data when first loaded
        setOriginalBenchmarkData(data.benchmark_table)
      }
    } catch (e) {
      console.error(e)
  ...
  // Handle input change without affecting weights
  ...
  // Apply the input value to weights when confirmed
  const applyWeightChange = (key) => {
    const inputValue = inputValues[key]
    if (!inputValue) return

    const num = Number.parseFloat(inputValue)
    if (isNaN(num) || num < 0 || num > 1) {
      // Reset invalid input to current weight
      setInputValues((prev) => ({
        ...prev,
    ...
    // Update all input values to match new weights
    const newInputValues = {}
    Object.entries(newWeights).forEach(([k, v]) => {
      newInputValues[k] = v.toFixed(2)
    })
  ...
  const handleSliderChange = (key, val) => {
    ...
    // Update the input value to match slider
    setInputValues((prev) => ({
      ...prev,
      [key]: num.toFixed(2),
    }))
  ...
  // Use originalBenchmarkData for dashboard tab and benchmarkData for weights tab
  const dataToDisplay = activeTab === "dashboard" ? originalBenchmarkData : benchmarkData
  ...
  const exportToExcel = () => {
    ...
    sortedData.forEach((item) => {
      const row = [capitalizeName(item.disease), item.benchmark_score]

      // Add breakdown scores if available
      if (item.score_breakdown_distribution) {
        row.push(
    ...
    XLSX.writeFile(wb, "disease-benchmark-data.xlsx")
  }
  ...
  const breakdownKeys = [
    ...
    ["Gut Microbiome Association", "Gut_Microbiome_Association"],
            ...
            <Tooltip>
              ...
              <TooltipContent className="max-w-xs rounded-[12px]">
                <p className="text-xs">
                  ...
                  Dashboard tab always shows original benchmark values.
                </p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
          ...
          <TabsContent value="dashboard" className="mt-0">
            <div className="p-4 flex justify-between items-center border-b">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  variant="outline"
                  className="h-8 border-[#a6ce39] text-[#6b8e23] rounded-[12px] focus:outline-none"
                  ...
                  <div key={k} className="space-y-1">
                    <div className="flex justify-between">
                      ...
                      <div className="flex items-center gap-1">
                        <input
                          type="text"
                          value={inputValues[k] || v.toFixed(2)}
                          ...
                          onBlur={() => applyWeightChange(k)}
                          className="w-12 h-5 text-xs text-right border rounded px-1 focus:outline-none focus:ring-1 focus:ring-[#a6ce39]"
                          disabled={lockedWeights[k]}
                        />
                        <button
                          className="text-xs bg-[#a6ce39] text-white px-1 rounded hover:bg-[#95b933]"
                          onClick={() => applyWeightChange(k)}
                          disabled={lockedWeights[k]}
                        >
                          Set
                        </button>
